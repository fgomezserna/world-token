...
export async function main(taskArgs: any) {
    ...
    let { contratovesting, wallet, amount, starttime, duration, percenttostart, cliff } = taskArgs;
    ...
    if(percenttostart > 0) {
        ...
        const createVestingScheduleTx = await vestingContract.createVestingSchedule(
            ...
            starttimeUnix,
            0,
            DurationUnits.Days, // Asumiendo que la duración está en días
        ...
        const vestingSchedule = await vestingContract.vestingSchedules(wallet,numSchedules-1);
        formatForConsole(vestingSchedule);
    }
    if(typeof cliff === 'string') {
        starttime = cliff;
    }
